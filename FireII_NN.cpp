...
g++ -O2 -std=c++11 -fopenmp -mbmi2 FireII.cpp -o FireII
...
double acc(double Y[OUTPUT]){//他クラス値分類
    
    return (Y[0]-(double)data_pl)*(Y[0]-(double)data_pl)/2.0;
}
...
double minl;
    ...
    for(int j=0;j<ROW*COL;j++){
        if(field[j/COL][j%COL]>0){conv[j+(ROW*COL)*(int)(field[j/COL][j%COL]-1)]=1;}
    }
			...
			for (int j = 0; j < DIR; j++) {//上下左右の4方向が発生
				...
				if (0 <= cand.nowC + dx[j] && cand.nowC + dx[j] < COL &&
					...
					if (cand.prev + j != 3) {
						...
						st = omp_get_wtime();
						sc cmb;
						cand.score = evaluate3(dropBB, EVAL_FALL | EVAL_COMBO, &cmb,p_maxcombo)+500;
						if(is_test){
						double X[INPUT]={0};
...
int main() {
	...
	for (i = 0; i < TEST; i++) {//PROBLEM問解く
		...
		*/
		...
		ll BB[DROP+1];
		operation(field, tmp.first_te,tmp.moving,BB);
