...
//コンパイル
Linux:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread make_traindata.cpp loguru.cpp -o make_traindata -mcmodel=large -ldl
Windows10,Windows11:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread make_traindata.cpp loguru.cpp -o make_traindata -mcmodel=large
MacOS:g++ -std=c++11 -fopenmp -mbmi2 -lpthread make_traindata.cpp loguru.cpp -o make_traindata -ldl
...
//printf("ddepth2=%d/%d\n",i+1,MAX_TRN);
		for (int j = 0; j < DIR * ks; j++) {
			...
			int scor=fff[j].score+(BONUS*fff[j].improving)+(((fff[j].pos)/COL)*3)+2000;
//printf("scor=%d\n",scor);
			vec[scor].push_front(j);
			ret_score=max(ret_score,scor);
			ks2++;
			}
		}
		part2+=omp_get_wtime() - start;
		if(congrats){return ret_score;}
		start = omp_get_wtime();
		int push_node=0;
...
void BEAM_SEARCH2(F_T f_field[ROW][COL],int MAX_TRN); //ルート探索関数
void BEAM_SEARCH2(F_T f_field[ROW][COL],int MAX_TRN) {
			...
			for (int j = 0; j < DIR; j++) {//上下左右の4方向が発生
				...
				if (0 <= xx + dx[j] && xx + dx[j] < COL &&
					...
					if (cand.prev + j != 3) {
						...
						cand.score = evaluate3(dropBB, EVAL_FALL | EVAL_COMBO, &cmb,p_maxcombo);
//printf("koko1");
						cand.score = BEAM_SEARCH(field,i+2,5,j,cand.pos,stop);
//printf("koko2");
						memcpy(cand.board,field,sizeof(field));
						cand.combo = cmb;
						//}
						//part1 += omp_get_wtime() - st;
						cand.prev = j;
...
//printf("idx=%d/%d\n",(DIR*k)+j,NODE_SIZE);
		...
		for (int j = 0; j < DIR * ks; j++) {
			if (iii[j].combo != -1) {
			if (iii[j].combo >= stop) {
				maxValue = iii[j].combo;
				bestAction.score = maxValue;
				bestAction.first_te = iii[j].first_te;
				memcpy(bestAction.moving, iii[j].movei, sizeof(iii[j].movei));
				hash_chain hc;
				F_T abc[ROW][COL];
				memcpy(hc.field,f_field,sizeof(abc));
				hc.first_te=iii[j].first_te;
				memcpy(hc.movei, iii[j].movei, sizeof(iii[j].movei));
				hc.calc_hashchain();
				if((int)hc.hashchain.size()>0){
				for(int r=0;r<(int)hc.hashchain.size()-1;r++){
				ll cur=hc.hashchain[r];
			...
			if(iii[j].score>iii[j].prev_score){iii[j].improving=iii[j].improving+1;}
			iii[j].prev_score=iii[j].score;
			int scor=iii[j].score+(BONUS*iii[j].improving)+(((iii[j].pos)/COL)*3)+2000;
			vec.push_back(make_pair(scor,j));
			//vec[fff[j].score+(BONUS*fff[j].improving)+(((fff[j].pos)/COL)*3)+2000].push_front(j);
			memo(iii[j].board,iii[j].score);
			ks2++;
			}
		}
		if(congrats){return;}
