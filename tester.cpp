...
g++ -O2 -std=c++11 -fopenmp tester.cpp -o tester
...
#define PROBLEM 100//koko//test時100,学習時1000000
#define BONUS 10//評価値改善係数
#define MAX(a, b) ((a) > (b) ? (a) : (b))
#define NODE_SIZE MAX(500,4*BEAM_WIDTH)
#define TRAIN -1//koko,test時-1,学習時1000000
	 ...
	 for(int i=0;i<10;i++){
         for(int j=0;j<(int)v[i].size();j+=3){
            ...
            for(int c=i+1;c<10;c++){
                for(int k=0;k<(int)v[c].size();k+=3){
                    if((int)v[c].size()<=k+2){break;}
                    int q1 = v[c][k];
                    int q2 = v[c][k+1];
                    int q3 = v[c][k+2];
                    int d3 = q2 - q1;
                    int d4 = q3 - q1;
                    ev+=data2[i][c][d1][d2][d3][d4];
            }
        }
    }
    }  
...
int main() {
	...
	if(start_test){
	    ...
	    while(getline(myf,ls)){
            ...
            if(stoi(child)<=10){data2[stoi(xz[0])][stoi(xz[1])][stoi(xz[2])][stoi(xz[3])][stoi(xz[4])][stoi(xz[5])]=0;}
		    }
		myf.close();
	}
	...
	for (i = 0; i < PROBLEM; i++) {//PROBLEM問解く
        ...
        /*
		if(i%100==0&&!go&&i>0){//koko,学習時実体化,test時コメントアウト
 		ofstream fi("data6.txt");
		string mystr="";    
